...
AI safety researcher and engineer with 6+ years developing production AI systems and leading engineering teams. Independent researcher with published work on activation steering, model interpretability, and LLM inference optimization. Currently researching evaluation awareness in large language models.
...
\begin{itemize}[leftmargin=*, nosep]
\item Co-developed EvalAwareBench, an open benchmark that measures whether frontier LLMs can distinguish evaluation from deployment contexts, designed to work with any Inspect-compatible eval. Paper under review.
\item Uncovered previously unidentified sources of measurement bias in eval-awareness benchmarks through controlled  experiments, and developed corrections now integrated into the pipeline.
\end{itemize}
...
\textbf{AI-Powered Sales Chatbot} \hfill \textbf{Nov 2024 -- Dec 2025} \\
...
\item Refactored existing codebase from a proprietary framework to LangGraph, significantly reducing overall codebase size.
...
\textit{Team Lead, ML Engineer, Backend Developer}
\begin{itemize}[leftmargin=*, nosep]
\item Designed and implemented semi-autonomous data analysis platform using AutoGen framework with custom modifications.
...
\begin{itemize}[leftmargin=*, nosep]
\item Led complete architecture modernization, migrating from bare-metal infrastructure to containerized Kubernetes deployment.
\item Rewrote legacy R-based signal generation in Scala using Akka actors for improved performance and maintainability.
\item Enabled sub-second portfolio visualization and metric calculation for real-time trading decisions.
\end{itemize}

\textbf{ML Model Operationalization Platform} \hfill \textbf{2019 -- 2021} \\
\textit{Backend Developer, MLOps Engineer}
\begin{itemize}[leftmargin=*, nosep]
\item Built automated retraining pipelines for image classification models with drift detection and performance monitoring.
\item Engineered ML model deployment service supporting DAG-based workflows with canary and blue-green strategies.
\item Developed Java microservices ecosystem for dynamic Jupyter notebook provisioning and execution.
\end{itemize}
...
\item Developed \href{https://github.com/arambharadwaj/STU-PID}{STU-PID}, a novel activation steering technique achieving 32\% token reduction and improved reasoning accuracy on GSM8K benchmark. Published as \href{https://arxiv.org/abs/2506.18831}{Steering Token Usage with PID Control}.
\end{itemize}
